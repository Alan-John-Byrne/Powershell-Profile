...
function Edit-PowerShell-Profile { code $PROFILE }
...
function Show-Profile-Aliases { bot $PROFILE 18 }
...
function PowerShell-Package-Manager {
    ...
    switch ($command) {
        ...
        "installed" { 
            ...
            $installedChocoPackages = choco list --no-color | Select-String "([^ ]+)\s+(\d+\.\d+\.\d+[^\s]*)" | ForEach-Object {
